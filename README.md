...
A fault-tolerant, distributed key-value store built in Go, featuring a **from-scratch implementation of the Raft consensus protocol**. Data is replicated across a cluster of nodes and survives node failures with zero data loss.

This project was built to deeply understand distributed systems, consensus algorithms, and the challenges of implementing the Raft paper (Diego Ongaro and John Ousterhout) without relying on external libraries like `hashicorp/raft`.
...
```bash
# Terminal 1
./raft-kv.exe --node-id=node1 --http-addr=:8001 --raft-addr=:9001 --peers=:9001,:9002,:9003

# Terminal 2
./raft-kv.exe --node-id=node2 --http-addr=:8002 --raft-addr=:9002 --peers=:9001,:9002,:9003

# Terminal 3
./raft-kv.exe --node-id=node3 --http-addr=:8003 --raft-addr=:9003 --peers=:9001,:9002,:9003
```
